...
TRACKING_STATE_PREFIX = "cv:ml:tracking-state"
TRACKING_RESULT_CACHE_PREFIX = "cv:ml:tracking-result"


def _actor_name(task_id: Any) -> str:
    return f"yolo-tracking-{str(task_id).replace('-', '')}"


def get_tracking_actor(ray: Any, settings: Settings, task_id: Any) -> Any:
...
class RealTrackingHandler:
    ...
    async def _infer(
        ...
        actor = await self.actors.get(job.task_id)
