...
- a gradual transition is emitted only by the chunk whose valid range contains
  its `end_frame`; if the many-hot range is still open at a non-final chunk
  boundary, its start/confidence are persisted and the event is emitted by the
  next chunk only after the real end is known;
- scene intervals returned for tracking must be clipped to the valid range.

If the deferred range ends exactly on the previous boundary, the next chunk
owns that one gradual event and starts directly with the new scene. These rules
prevent both missed boundary events, duplicate transitions, and premature
ByteTrack resets.
...
Final result version `1.2` includes measured queue, decode, cut, tracking,
aggregation, orchestration/wait, total wall-clock timings, and merged scene
intervals for API clients. Active stage
...
publication, which makes redelivery idempotent. The one-hot output head emits
point `hard_cut` events; the many-hot output head is decoded into inclusive
`gradual_transition` ranges. A many-hot range is accepted only when it is long
enough and a sufficiently confident one-hot peak lies near its midpoint. This
dual-head confirmation rejects motion/lighting bursts that only activate the
range head. A hard-cut peak inside an accepted gradual range is suppressed so
one visual transition cannot split the scene twice. Only transitions whose
global anchor belongs to the chunk valid range are emitted.

The configured YOLO-World detector uses a named actor per task, which is
the state partition required by ByteTrack. The actor processes only scene
intervals inside the valid range,
resets after every verified transition (and defensively on a changed
`scene_id`), maps raw tracker IDs to scene-local IDs, and
...
Ingest admits two chunks per task in the real-model profile. Because tracking
chunk N waits only for N's verified cut result, AutoShot may process N+1 at the
same time while per-video cut ordering and ByteTrack ordering remain strict.
ByteTrack receives low-confidence candidates for its second association pass,
but JSON emission uses the higher detector threshold and new tracks require a
...
partially matched weights. Gradual detection uses an independent confidence
threshold, one-hot confirmation threshold, minimum range length, and short-gap
merge setting; all four are validated backend settings and passed into the
named Ray actor. Tracking
expects the local `models/best.pt`
YOLO-World checkpoint plus `models/yoloe26l_military_assets.pt`, loads both
with the pinned Ultralytics 8.4.112 runtime, verifies the primary `WorldModel`,
and installs the selected vocabulary with `set_classes()`. Both detectors run
on the same frame; class-aware NMS merges duplicate same-class boxes before a
single ByteTrack update. The UI vocabulary contains COCO-80 plus 12 military
labels, while auxiliary detections outside the selected vocabulary are ignored.
