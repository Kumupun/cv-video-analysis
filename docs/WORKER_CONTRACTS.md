...
```json
{
  ...
  "transitions": [
    ...
    },
    {
      ...
      "start_frame": 110,
      "end_frame": 116,
      "start_timestamp": 3.6666667,
      "end_timestamp": 3.8666667,
      "confidence": 0.82
    }
  ],
  "scenes": [
    {"scene_id": "task:scene:5", "start_frame": 64, "end_frame": 92},
    {"scene_id": "task:scene:6", "start_frame": 93, "end_frame": 116},
    {"scene_id": "task:scene:7", "start_frame": 117, "end_frame": 127}
  ]
}
```
...
## Contract additions in pipeline versions 1.1 and 1.2
...
Version `1.2` adds optional `tracking_classes` to `TrackingJobMessage`. The API
validates the frontend's multipart `classes` JSON against the checkpoint
vocabulary, stores it with `VideoSource`, and the coordinator copies it into
every tracking job. `null` uses the server default, `[]` intentionally skips
object tracking, and a non-empty list filters the YOLO inference classes. The
final result also includes merged `scenes` intervals in addition to transitions
and flattened tracks.
