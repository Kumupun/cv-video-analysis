...
6. `backend/app/workers/mock_cut.py` та `mock_tracking.py` — лише development-симулятори контрактів. Реальний ML-профіль використовує AutoShot та ансамбль YOLO-World + YOLOE з одним ByteTrack.
...
participant_3_tracking/      # YOLO-World + YOLOE ensemble, ByteTrack and Redis adapter
...
frontend/                    # static UI served by FastAPI on the same origin
...
`classes` is an optional JSON array inside the multipart form. The frontend
offers 92 YOLO-World prompts: the standard COCO-80 vocabulary plus the 12
military labels used by the previous custom checkpoint. The selected labels
are installed through `set_classes()` before inference. An omitted field uses
the server default; an empty array runs cut detection without object tracking.
...
AutoShot уже доданий у `models/weights_for_cut.pth`. Перед реальним запуском
додайте наданий другом YOLO-World checkpoint як `models/best.pt` та залиште
`models/yoloe26l_military_assets.pt` як другу модель ансамблю, після чого
запускайте **без** профілю `mock`:
...
`ml-ray-worker` є справжнім GPU Ray node. AutoShot і tracking actors
...
зберігає checkpoint ByteTrack-стану в Redis між chunk-ами. Конвеєр тримає два
chunk-и: поки ByteTrack обробляє `N`, AutoShot уже може перевіряти `N+1`.
Tracking конкретного chunk-а все одно починається лише після його cut-result.
...
Python traceback. Worker використовує обидва виходи AutoShot: one-hot head
визначає точкові `hard_cut`, а many-hot head — діапазони
`gradual_transition`. Пороги та мінімальна довжина налаштовуються через
`CUT_MODEL_THRESHOLD`, `CUT_MODEL_GRADUAL_THRESHOLD`,
`CUT_MODEL_GRADUAL_BOUNDARY_THRESHOLD`, `CUT_MODEL_GRADUAL_MIN_FRAMES` і
`CUT_MODEL_GRADUAL_MERGE_GAP_FRAMES`. Gradual range приймається лише коли
many-hot діапазон підтверджений one-hot піком біля його середини. Точніші
production defaults: `0.75`, `0.65`, щонайменше 6 кадрів і без об’єднання
розірваних діапазонів.
Реальний ML-профіль тримає два in-flight chunk-и для
асинхронного перекриття AutoShot і ByteTrack; відео з ZIP залишаються
...
ByteTrack використовує окремий конфіг. За замовчуванням detector віддає tracker-у
кандидатів від `0.10`, перша асоціація працює від `0.50`, новий ID створюється від
`0.55`, а втрачений track зберігається `45` кадрів. У фінальний JSON потрапляють
лише детекції від `TRACKING_MODEL_CONFIDENCE`. Після будь-якого transition кадр
склейки завершує стару сцену, predictor та ID-map скидаються, а наступний кадр
починає track заново.

Для `model.set_classes()` ML-образ містить офіційний OpenAI CLIP із закріпленого
commit. Його `ViT-B/32` text encoder завантажується та перевіряється під час
`docker build`, а не під час обробки користувацького відео.
...
- Не вмикайте `mock` profile у production. Перед ML-запуском перевірте AutoShot через `sha256sum -c models/SHA256SUMS` і окремо звірте hashes локальних `best.pt` та `yoloe26l_military_assets.pt` з власниками ваг; runtime download вимкнено.
...
Redis/Ray/backpressure overhead. Result contract `1.2` additionally returns the
merged `scenes` list used by the frontend and preserves the per-task tracking
class selection through the coordinator.
